...
def draw_polygons( polygons, screen, color ):
    ...
    while (i<len(polygons)-2):
        if (calculate_normal(polygons,i)[2]>0):
            ...
            draw_line(int(p2[0]), int(p2[1]), int(p0[0]), int(p0[1]), screen, color)
        i+=3
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop+1):
            ...
            add_polygon(polygons,
                        ...
                        points[(index+1)%len(points)][2],points[(index+steps+1)%len(points)][0],
                        points[(index+steps+1)%len(points)][1],points[(index+steps+1)%len(points)][2])

            if ((longt!=0) and (longt!=(steps-1))):
                add_polygon(polygons,
                            points[index][0],points[index][1],points[index][2],
                            points[(index+steps+1)%len(points)][0],points[(index+steps+1)%len(points)][1],
                            points[(index+steps+1)%len(points)][2],points[(index+steps)%len(points)][0],
                            points[(index+steps)%len(points)][1],points[(index+steps)%len(points)][2])
